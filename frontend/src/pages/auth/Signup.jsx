...
const Signup = () => {
  const { BASE_URL, isLoggedIn, setIsLoggedIn, } = useContext(AuthContext)
    ...
    <div className='max-w-7xl min-h-screen mx-auto flex flex-col items-center justify-center space-y-2'>
      ...
      <form onSubmit={handleSubmit} className='border border-neutral-300  px-4 py-3 w-100 flex flex-col space-y-4 shadow-sm bg-[#fefffe]'>
        ...
        <div className='flex flex-col items-start justify-start gap-2'>
          ...
          <input onChange={handleChange} className='w-full px-3 py-2 border border-neutral-300  outline-none text-sm' type="email" name='email' placeholder='you@example.com' value={formData.email} />
        </div>
        <div className='flex flex-col items-start justify-start gap-2'>
          <label className='text-sm font-medium flex items-center gap-1'><Calendar size={14} />Age</label>
          <input onChange={handleChange} className='w-full px-3 py-2 border border-neutral-300  outline-none text-sm' type="number" name='age' placeholder='Age' value={formData.age} />
        </div>
        <div className='flex flex-col items-start justify-start gap-2'>
          <label className='text-sm font-medium flex items-center gap-1'><CalendarFold size={14} />Date of birth</label>
          <input onChange={handleChange} className='w-full px-3 py-2 border border-neutral-300  outline-none text-sm text-neutral-800' type="date" name='dob' placeholder='John Doe' value={formData.dob} />
        </div>
